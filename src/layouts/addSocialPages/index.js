...
// const REDIRECT_URI = "https://marketincerfe-master-1.onrender.com/social";
...
const Index = () => {
  ...
  const fetchAccountsFromAPI = async (authCode) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://marketincer-apis.onrender.com/api/v1/social_accounts/get_pages`, {
        method: "POST",
        headers: {
  ...
  const handleConnect = async (account) => {
    ...
    try {
      ...
      const response = await fetch("https://marketincer-apis.onrender.com/api/v1/social_pages/connect", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
